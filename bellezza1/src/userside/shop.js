...
function Shop() {
  ...
  const handleProductClick = async (id) => {
    try {
      const response = await axios.get(`${backendUrl}/users/detailproduct/${id}`);
  ...
  useEffect(() => {
    axios
      .get(
        `${backendUrl}/users/pageproduct?page=${currentPage}&pageSize=${pageSize}&category=${selectedCategory}&tag=${selectedTag}&brand=${selectedBrand}&material=${selectedMaterial}&pricerange=${selectedPriceRange.min}-${selectedPriceRange.max}&sortBy=${sortBy}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productListResponse = await axios.get(`${backendUrl}/users/listproduct`);
        setLists(productListResponse.data);

        const [categoryResponse, tagResponse, brandResponse, materialResponse] = await Promise.all([
          axios.get(`${backendUrl}/users/listcategory`),
          axios.get(`${backendUrl}/users/listtag`),
          axios.get(`${backendUrl}/users/listbrand`),
          axios.get(`${backendUrl}/users/listmaterial`),
        ]);
  ...
  const handleAddToWishlist = (product) => {
    const wishItem = {
      ...
      image: `${backendUrl}/uploads/${product.images[0]}`,
      quantity: 1,
    };
                    ...
                    >
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${backendUrl}/uploads/${item.images[0]})`,
                          }}
                        >
