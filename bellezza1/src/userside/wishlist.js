...
function Wishlist() {
  ...
  const handlePassword = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${backendUrl}/users/changepassword/${userDetails.id}`,
        formData
      );
  ...
  const handleCancelProductInOrder = async (orderId, productId) => {
    ...
    try {
      const response = await axios.post(`${backendUrl}/users/cancelorder/${orderId}`, {
        productId: productId,
      });
      if (response.data.success) {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(`${backendUrl}/users/detailorders/${userDetails.id}`);
  ...
  const handleSubmit = (e) => {
    ...
    try {
      const response = axios.post(`${backendUrl}/users/edituser/${userDetails.id}`, formData);
