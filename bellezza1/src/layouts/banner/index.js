...
function Banner() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/admin/listbanner`);
  ...
  const handleEditBanner = (e) => {
    ...
    if (image) {
      ...
      reader.onloadend = async () => {
        ...
        axios
          .put(`${backendUrl}/admin/editbanner/${selectedProduct._id}`, formData)
          .then((response) => {
            console.log(response);
  ...
  const deleteBanner = async (id) => {
    try {
      await axios.delete(`${backendUrl}/admin/removebanner/${id}`);
