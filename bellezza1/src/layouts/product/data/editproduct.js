...
function Editproduct() {
    ...
    if (state) {
      ...
      if (image) {
        ...
        for (let i = 0; i < image.length; i++) {
          const imageURL = `${backendUrl}/uploads/${image[i]}`;
          previewImages.push(imageURL);
        }
        setImagePreview(previewImages);
        // setImage([image]);
        // setImagePreview([`${backendUrl}/uploads/${image}`]);
      } else {
        setImage([]);
        setImagePreview([]);
      }
  ...
  useEffect(() => {
    axios
      .get(`${backendUrl}/admin/listcategory`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`${backendUrl}/admin/listbrand`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`${backendUrl}/admin/listmaterial`)
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`${backendUrl}/admin/editproduct/${id}`, formData, {
        headers: {
